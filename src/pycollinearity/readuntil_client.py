...
class ReadUntilClient(object):
    ...
    def __init__(
        ...
        info("Strand-like classes are %s.", self.strand_classes)
    ...
    def reset(self):
        ...
        if self._process_thread is not None:
            info("Reset request received, shutting down...")
            self.running.clear()
            self._process_thread.join()  # block, try hard for .cancel() on stream
            if self._process_thread.is_alive():
                warn("Stream handler did not finish correctly.")
            else:
                info("Stream handler exited successfully.")
        self._process_thread = None
    ...
    def _run(self, **kwargs):
        ...
        try:
            ...
            if code in {code.CANCELLED, code.ABORTED, code.FAILED_PRECONDITION}:
                info(f"Read stream finished due to {code}")
            else:
                error("Failed Processing reads:")
    ...
    def _runner(
        ...
        if min_chunk_size > ALLOWED_MIN_CHUNK_SIZE:
            warn("Reducing min_chunk_size to %s", ALLOWED_MIN_CHUNK_SIZE)
            min_chunk_size = ALLOWED_MIN_CHUNK_SIZE
        ...
        info("Sending init command, parameters: %s", setup)
        ...
        while self.is_running:
            try:
                ...
                info("Sending %s actions.", len(actions))
    ...
    def _process_reads(self, reads):
        ...
        for reads_chunk in reads:
            if not self.is_running:
                info("Stopping processing of reads due to reset.")
                break
            ...
            for read_channel in reads_chunk.channels:
                ...
                if self.one_chunk:
                    if read.id in unique_reads:
                        ...
                        #   we reinsert.
                        info(
                            "Received %s:%s after stop request.",
                            read_channel,
                            read.id,
                        )
                        continue
            ...
            if last_msg_time + log_interval_secs < now:
                ...
                info("Response summary: %s", response_counter)
